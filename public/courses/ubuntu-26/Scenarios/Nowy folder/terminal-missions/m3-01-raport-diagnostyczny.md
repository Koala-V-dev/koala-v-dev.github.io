...
- w historii widać użycie potoku i zapisu do pliku.</objective>
  ...
  <assessment min-evidence="1">
    <evidence id="report-dir" label="Utworzono katalog raportu `Raport-INF02`">
      <expect exists="Raport-INF02" type="dir"></expect>
    </evidence>

    <condition id="hardware-report" label="Zapisano modele sprzętu. W pliku `Raport-INF02/sprzet.txt`">
    ...
    <condition id="disk-report" label="Zapisano dyski i zajętość miejsca. W pliku `Raport-INF02/dyski.txt`">
    ...
    <condition id="memory-report" label="Zapisano informacje o pamięci. W pliku `Raport-INF02/pamiec.txt`">
      <expect exists="Raport-INF02/pamiec.txt" type="file" file="Raport-INF02/pamiec.txt" file-contains-all="Mem:,Swap:"></expect>
    </condition>

    <condition id="network-process" label="Zapisano proces sieciowy. W pliku `Raport-INF02/proces-siec.txt`">
