import{T as l,S as c}from"./ScrollManager-QveMOUhZ.js";import"./markdown-CUzkdAAO.js";let i=[];document.addEventListener("DOMContentLoaded",async()=>{const t=document.getElementById("header-actions");t&&new l().inject(t),new c,await d(),document.body.classList.add("app-bootstrapped")});async function d(){const t=document.getElementById("home-courses-grid-mount");if(t)try{const e=await fetch("/courses/registry.json");if(!e.ok)throw new Error(`HTTP ${e.status}`);i=((await e.json()).courses||[]).filter(a=>a.status==="published"),u(i,t)}catch(e){console.info("[Koala-V] Brak registry.json lub błąd pobierania.",e.message),t.innerHTML='<div class="courses-empty"><p>Kursy są w przygotowaniu — wkrótce tutaj!</p></div>'}}function u(t,e){if(!t.length){e.innerHTML='<div class="courses-empty"><p>Brak dostępnych kursów.</p></div>';return}e.innerHTML=`
    ...
    `;const s=document.getElementById("search-input");s&&s.addEventListener("input",a=>p(a.target.value.trim()))}function n(t){return t.map(e=>{const s=e.thumbnail||"/Resources/courses/default_course.webp";return`
        ...
        `}).join("")}function p(t){const e=document.getElementById("courses-list");if(!e)return;const s=t.toLowerCase(),a=s?i.filter(r=>r.title?.toLowerCase().includes(s)||r.description?.toLowerCase().includes(s)||r.tags?.some(o=>o.toLowerCase().includes(s))):i;a.length?e.innerHTML=n(a):e.innerHTML='<li style="grid-column: 1 / -1; text-align: center; color: var(--text-muted);">Brak kursów spełniających kryteria.</li>'}
